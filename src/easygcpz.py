"""EasyGCPz takes a significant load off of technical users of GCP by
automating query generation, executing queries, and returning query
results in a number of optional accessible data structures. It
offers multiple degrees of flexibility in its use while also being
robust against a wide number of common querying and usage problems."""
__author__ = 'MW-OS'
__version__ = 0.1
__date__ = '2024.03'
...
# TODO add lists+ to output data structures
# --> list of lists, tuples, arrays, series
# list, list_tuple, list_array, list_series

class EasyGCPz:
    """A class that contains all of EasyGCPZ uses and features.
    ...
    query(queries,
          ...
          **kwargs)
    -> Executes a query, optionally iterating it, optionally saving results.

    iterated_query(query_in,  **kwargs)
...
# -------- connection methods --------
    ...
    def connection(self, connection_info):
        ...
        try:
            ...
            if connection_json is None or connection_project is None:
                ...
                self._connection = connection_json + connection_project
        ...
        # establish current connection data and log it
            ...
            self.log = (f'ATTEMPT CONNECTION SETTING: '
                        f'{str(connection_json)} + '
                        f'{str(connection_project)}')
    ...
    def query(self,
        ...
        if kwargs != {}:
            ...
            self._query_inputs_validate(queries,
                                        return_ascii,
                                        return_format,
                                        file_same,
                                        file_separate)
    ...
    def _query_exec(self,
        ...
        try:
            ...
            # TODO DELETE THIS AFTER TESTING
            return str(self._connection) + str(query_in)
            # raise EasyGCPzException(
            #     f'EasyGCPz has encountered a GCP the following error: '
            #     f'\n-->{e} \n-\n'
    ...
    def _query_out_processor(self,
        ...
        if file_same != '':
            self.log = f'SAVING OUTPUT TO: {file_same}'
        ...
        if file_separate != '':
            ...
            self.log = f'SAVING RESPECTIVE QUERY TO: {file_separate_query}'
        ...
        if return_format == 'dict':
            ...
            return {i: pd.Series(results[i]) for i in results.keys()}

        elif return_format == 'dict_array':
            return {i: np.array(results[i]) for i in results.keys()}

        elif return_format == 'dataframe':
            return pd.DataFrame(results)

        elif return_format == 'array':
            return np.array(list(results.values()), dtype=object)

        elif return_format == 'structured_array':
            # initialize structured array with generic object type since
            # queries could return any number of types if return_ascii
            # is False
    ...
    def _query_inputs_validate(queries,
        ...
        # TODO make kwargs processor

        # raise formatted exception if any issues exist
        if len(issue_list) > 0:
            issue_str = ' \n'.join(f'{i[0]}. \n- {i[1]}' for i in issue_list)
            raise EasyGCPzException(f'Issues encountered in the inputs: \n'
                                    f'{issue_str}')
    ...
    def iterated_query(query_in,  **kwargs):
        ...
        for i in range(len(iterable_dict.keys())):
    ...
    def save_log(self, path_out):
        ...
        :param path_out: Write path to log file.
        """
        if not isinstance(path_out, (str, pathlib.PurePath)):
            print('save_log function requires the path_out arguement
                  'to be of string or pathlib type')
            return
        ...
        with open(path_out, 'w+', newline='') as f:
            # add header and convert from column centric data to
            # column centric
            csv.writer(f).writerows(self.log)
    ...
    def _try_str_float(obj_in):
        ...
        obj_out = np.squeeze(np.array(list(obj_in), dtype='str_'))
        try:
            return obj_out.astype(float).tolist()
        except ValueError:
            return obj_out.tolist()
...
if __name__ == '__main__':
    ...
    tmp = EasyGCPz('testjson', 'testproj')

    with tmp as t:
        print(t('not a quiery1', 'not a quiery2'))
